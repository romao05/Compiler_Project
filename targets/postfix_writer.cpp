...
{
  ASSERT_SAFE_EXPRESSIONS;
  bool isTakum3 = node-> left() -> is_typed(cdk::TYPE_TAKUM3)
    || node-> right() -> is_typed(cdk::TYPE_TAKUM3);
  node->left()->accept(this, lvl);
  if (isTakum3 && node->left()->is_typed(cdk::TYPE_BALANCED3)) _pf.B2T();
  node->right()->accept(this, lvl);
  if (isTakum3 && node->right()->is_typed(cdk::TYPE_BALANCED3)) _pf.B2T();
  _pf.LT();
}
...
void p6::postfix_writer::do_assignment_node(cdk::assignment_node *const node, int lvl)
{
  ...
  if (isTakum3) _pf.DUP128();   // 128 para 16 bytes
  else _pf.DUP64();       // 64 para 8 bytes 
  ...
  else
  {
    ...
    reset_new_symbol();
    if (isTakum3) _pf.STAKUM3 (cdk::takum3_type::value_type(0));
    else _pf.SBALANCED3(cdk::balanced3_type::value_type(0));                      // initialize it to 0 (zero)
    _pf.TEXT();                        // return to the TEXT segment
    node->lvalue()->accept(this, lvl); // DAVID: bah!
  }
  if (isTakum3) _pf.STTAKUM3();
  else _pf.STBALANCED3();
}
...
void p6::postfix_writer::do_input_node(p6::input_node *const node, int lvl)
{
  ...
  if (arg->is_typed(cdk::TYPE_POSIT3)) {
    arg->accept(this, lvl); // push destination address
    _pf.CALL("posit3_read"); // posit3_read(posit3_t *result) — explicit sret
    _pf.TRASH(4); // pop the pointer argument
  } else if (arg->is_typed(cdk::TYPE_TAKUM3)) {
    arg->accept(this, lvl); // push destination address
    _pf.CALL("takum3_read"); // takum3_read(takum3_t *result) — explicit sret
    _pf.TRASH(4); // pop the pointer argument
  } else {
    _pf.CALL("balanced3_read");
    _pf.LDFVAL64I(); // balanced3 result is returned in eax:edx
    arg->accept(this, lvl);
    _pf.STBALANCED3();
  }
}
